<?php

/**
 * @package    theme
 * @subpackage marinada
 * @copyright  2013 Fernando Acedo. http://3-bits.com
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */

 
 
$string['configtitle'] = 'Marinada';
$string['pluginname'] = 'Marinada';

// Settings page

$string['region-side-post'] = 'Right';
$string['region-side-pre'] = 'Left';
$string['purgecaches'] = 'Purge caches';
$string['notes'] =  'Bootstrap Configuration page';
$string['notesdesc'] = 'Configure your theme';
$string['logo_url'] = 'Logo URL';
$string['logo_urldesc'] = 'Enter the full URL of your custom logo, make sure it is of a proper size (300 x 100 px)';
$string['logo2_url'] = 'Logo 2 URL';
$string['logo2_urldesc'] = 'Enter the full URL of your custom logo 2, make sure it is of a proper size (300 x 100 px)';
$string['customcss'] = 'Custom CSS';
$string['customcssdesc'] = 'Add your extra CSS in this box, use it to customize your theme with a little css';

$string['bodybgcolor'] = 'Body background color';
$string['bodybgcolordesc'] = 'Body background color';

$string['linkcolor'] = 'Link color';
$string['linkcolordesc'] = 'Link color';

$string['linkhovercolor'] = 'Link hover color';
$string['linkhovercolordesc'] = 'Link hover color';

$string['menubgcolor'] = 'Menu background color';
$string['menubgcolordesc'] = 'Menu background color';

$string['topmenubgcolor'] = 'Top menu background color';
$string['topmenubgcolordesc'] = 'Top menu background color';

$string['topmenulinkcolor'] = 'Top menu link color';
$string['topmenulinkcolordesc'] = 'Top menu link color';

$string['topmenulinkhovercolor'] = 'Top menu link hover color';
$string['topmenulinkhovercolordesc'] = 'Top menu link hover color';

$string['footnote'] = 'Footnote';
$string['footnotedesc'] = 'The content from this textarea will be displayed in the footer of every page.';


// Theme info
$string['choosereadme'] = 
'<div class="clearfix"><div class="theme_screenshot"><h2>Marinada</h2>
<img src="marinada/pix/screenshot.png" />
<h3>Theme Credits</h3><p>
This theme is based in the Bootstrap theme created by: Bas Brands <br />

<h2>License</h2><p>This theme is licensed under the <a href="http://www.gnu.org/licenses/gpl.html">GNU General Public License</a>.</div></div>';

